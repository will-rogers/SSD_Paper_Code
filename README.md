...
All movement data are publically available and spatial data was collected from public sources, or are otherwise noted (see S2 of paper). All data and code will be archived upon acceptance, and are provided in the repository at the time of review. All data is provided as R data objects (or CSV for density data) for ease, but the movement data (besides that contained in amt) are available on MoveBank. A substantial amount of computational effort was performed on Yale HPC, and the analyses for ABMs would not be possible using local computational resources (e.g. 16GB RAM computer with 8 processors). We provide the scripts necessary to implement this pipeline on the HPS, but please contact if there are issues in implementation. Note: simulation SSF and RSF models and raw ABM simulations are not possible to provide here because of their size and Github limits. The precursors to and products from these objects are provided (we document below where these objects are missing in the analysis pipeline). Also, note that the African buffalo census data is sourced from SANParks Kruger National Parks Scientific Services. Reuse of this data for another purpose is prohibited, and inqueries about further use of this data should be directed to SANParks Kruger National Parks Scientific Services. In accordance with data sharing polices, we randomized the year associated with the spatial density data, as our results do not rely on the temporal structure of density but this allows us to acocunt for between-year density variation. 


To reproduce the results of the paper, you should only have to run the code within "Paper_Compilation.RMD" - this file will generate precursors for cluster simulations, and it pulls in cluster outputs as specified below. To run this code in RStudio is pretty straightforward, but make sure the file can properly access the "Simulation", "Casestudies", and "Code" subfolders. The code within this file should run with 10-15 minutes on a local PC with 16GB of RAM and 8 processors, but performance may vary. We also provide the compiled document as a HTML file (S3 in the text) for clarity.  
...
If you wish to recreate the cluster-based simulations, you need to load "base_fxns.R", "sur.val.packed.RDS", "sims.RDS", "index.sheet.csv", and "tidy_output.R" into cluster storage. Then, we recommend working through the slurm calls in this order - they should generate their own outdirectories locally, but this can be very tricky.
    ...
    8. run_ssf_map.slurm - generates naive SSF maps based on run_steps.slurm and run_ssf.slurm output
    9. run_ssd.slurm - generates SSD maps based on run_steps.slurm and run_ssf.slurm output
    10. tidy_output - ingests outputs of run_abm.slurm, run_rsf.slurm, and run_ssf.slurm to make smaller (storage) outputs

    run_abm_case.slurm:
        requires: "base_fxns.R", "sims.RDS", "reddeer.abm.RDS", "roedeer.abm.RDS", "fisher.focal.abm.RDS", "fisher.nonfocal.abm.RDS", "buffalo.abm.RDS"
...
The buffalo density data "buffalo_locations.csv" has four columns: 
    ...
    t.rand: the year of the detection
...
Case studies ABMs:
    ...
    run_abm_case.slurm:
        requires: "base_fxns.R", "reddeer.abm.RDS", "roedeer.abm.RDS", "fisher.focal.abm.RDS", "fisher.nonfocal.abm.RDS", "buffalo.abm.RDS"
        returns: "output_abm_case/" *Missing from files given size
