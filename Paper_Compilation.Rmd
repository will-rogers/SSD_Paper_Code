...
merged <- merge(spts, as.data.frame(pop.locations), by.x = c("x", "y"), by.y = c("V1","V2")) 
merged <- merged %>% 
  group_by(x,y) %>% 
  tally() 

pop.use <- rast(merged)
values(pop.use) <- ifelse(is.na(values(pop.use)), 0, values(pop.use))
plot(pop.use)
```
...
ggsave("Plots/BHAvsMapSize2.svg", width = 7, height = 4)
...
abmpredictions.red <- rast("Casestudies/abm_compiled/final_red_1.tif")
...
abm.raster.roedeer <- (abmpredictions.roe)
...
a. <- rbind(a, b) %>%
  ...
  geom_density(alpha = 0.5, color = NA, bw = 0.1) +
  facet_grid(~group) +
  scale_y_sqrt() +
  theme_classic() +
...
d. <- rbind(a, b) %>%
  ...
  geom_density(alpha = 0.5, color = NA, bw = 2) +
  facet_grid(~group) +
  scale_y_sqrt() +
  theme_classic() +
...
focal <- rast(rstack.focal)
focal <- crop(focal, ext(focal.extent))

nonfocal <- rast(rstack.nonfocal)
...
values(abmpredictions.fisher) <- values(abmpredictions.fisher)
values(abmpredictions.fisher.nonfocal) <- values(abmpredictions.fisher.nonfocal)
...
# woody.cover <- rast("S1_A_VH_VV_mosaic_KNP_Area_no_nan_ranger_mosaic_woody_cover_S1_16_17_100m.tif") from https://doi.org/10.4102/koedoe.v62i1.1621 
# 
# growth <- project(growth, crs(boundaries))
# growth <- aggregate(growth, 2)
# 
# woody.cover <- project(woody.cover, crs(boundaries))
# woody.cover <- resample(woody.cover, growth, "average")
# 
...
# 
# dist.waterhole <- terra::distance(growth, vect(waterholes))
# dist.dams <- terra::distance(growth, vect(dams))
# 
...
# water.3 <- terra::distance(growth, streams.v[streams$Str_order == 3,])
# water.4 <- terra::distance(growth, streams.v[streams$Str_order == 4,])
# water.5 <- terra::distance(growth, streams.v[streams$Str_order == 5,])
# water.6 <- terra::distance(growth, streams.v[streams$Str_order == 6,])
# water.7 <- terra::distance(growth, streams.v[streams$Str_order >= 7,])
# 
...
values(abmpredictions) <- values(abmpredictions)
...
system.size <- tibble(system = c("roe","red","fisher.focal","fisher.nonfocal","buffalo"),
                       ncells = c(sum(!is.na(values(abmpredictions.red[[1]]))), 
